...
class LeaseQuoteIndexView extends Component {
    ...
    switchToListView = () =>{
        ...
        actions.multileft.updateState({ quoteFormObj : {},isGrid : true,isEdit : false, showForm : false});
    }
    ...
    switchToCardView = (obj) =>{
        ...
        actions.multileft.updateState({ quoteFormObj : _formObj,isGrid : false,isEdit : false, showForm : true});
    }
    ...
    switchEdit = () =>{
        ...
        actions.multileft.updateState({isEdit : !this.state.isEdit});
    }
    ...
    onAdd = () =>{
        //单击新增 要置空子表数据
        actions.multileft.updateState({list2:[]});
        let objectForm = localStorage.getItem("addKey");
        if(objectForm){
            let _formObject = deepClone(JSON.parse(objectForm));
            actions.multileft.updateState({quoteFormObj:_formObject});
        }else{
            
            //新增完成初始化form表单
            actions.multileft.updateState({quoteFormObj:{
                //租赁方式
                lease_method:'0',
                //本金是否开票
                if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.multileft.updateState({showModal : true});      
    }
    ...
    onView = () =>{
        //singleRecordOper(this.props.selectedList,(param) => {  //查看选中项数据前进行一次单选校验
            ...
            actions.multileft.updateState({bt:false});
        //});
    }
    ...
    onSave = () => {
        ...
        actions.multileft.updateRowData({'record':_formObj});
        this.switchEdit();
    }
