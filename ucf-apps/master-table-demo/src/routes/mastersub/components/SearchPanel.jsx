 ...
 * title 条件名称
 * key 取值字段
 * fixcon:true 默认点开搜索框 一直存在 不可移除的条件
 * type 类型
 */
const transData = [               //树节点搜索字段 使用假数据定义 后续从后端传入
      {
        title: "合同日期",
        key: "cont_date",
        _edit:true,
          ...
          // },
          {
            ...
            render: (text, record, index) => {
              if(record.type=='String'){
                ...
                return <div className = "ref_model"><RefModel text={text} record={record} index={index} dataIndex={'content'}/></div>
              }
            }
          },
];
