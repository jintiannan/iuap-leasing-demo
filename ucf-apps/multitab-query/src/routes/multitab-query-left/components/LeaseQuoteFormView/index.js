...
class LeaseQuoteFormView extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    add = () => {
        if (this.state.activeKey == '1') {
            actions.multileft.updateState({ planformObj: {}, ifplanAdd: true, showLoanModal: true })
        }
    }
    ...
    onEdit = () => {
        if (this.state.activeKey == '1') {
            singleRecordOper(this.props.selectedPlanList, (param) => {
                ...
                actions.multileft.updateState({ planformObj: _planformObj, showLoanModal: true });
            });
        }
    }
    //子表删除数据
    del = () => {
        if (this.state.activeKey == '1') {
            multiRecordOper(this.props.selectedPlanList, (param) => {
                ...
                actions.multileft.updateState({ list2: newlist, selectedPlanList: [] });
            });
        }
    }
    ...
    getSelectedDataFunc = (selectedPlanList, record, index) => {
        if (this.state.activeKey == '1') {
            ...
            actions.multileft.updateState({ list2: _list2, selectedPlanList: _selectedPlanList });
        }
    }
    ...
    render() {
        ...
        let formObject = deepClone(this.props.quoteFormObj);
...
export default Form.createForm()(LeaseQuoteFormView);