...
class LeaseQuoteListView extends Component {
    constructor(props) {
        super(props);
        this.state = {
            listView: '',
    ...
    componentWillMount() {
        //主表过滤显示字段
        const gridMain = this.getShowColumn([], this.grid, false);
        //主表定义字段调用公共方法初始化
    ...
    componentDidMount() {
        //查询后台数据
        actions.multileft.loadQuoteList({param:this.props.pkContract});
        //绑定父组件
    ...
    freshData = (pageIndex) => {
        ...
        actions.multileft.loadList(queryParam);
    }
    ...
    getSelectedDataFunc = (selectedList, record, index) => {
        ...
        if (_selectedList && _selectedList.length == 1) {
            ...
            actions.multileft.updateState({ });
        }
        actions.multileft.updateState({quoteFormObj: _formObj,quoteList:_list});

    }

    childList = (obj) => {
        //加载子组件列表
        actions.multileft.loadChildList(this.props.queryParam);
    }
            ...
            <div className="grid-parent" style={{ display: this.state.listView }}>
                 ...
                 */}
                    <GridMain
                        ...
                        data={this.props.quoteList} //数据数组                     
                        ...
                        paginationObj={{
                            ...
                            total: this.props.quoteList.length,//总条数
                            items: 1,//总页数
...
export default LeaseQuoteListView;