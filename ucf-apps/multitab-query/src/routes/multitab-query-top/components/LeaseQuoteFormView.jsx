...
class LeaseQuoteFormView extends Component {
    ...
    add = () => {
        if (this.state.activeKey == '1') {
            actions.multitop.updateState({ planformObj: {}, ifplanAdd: true, showLoanModal: true })
        }
    }
    ...
    onEdit = () => {
        if (this.state.activeKey == '1') {
            singleRecordOper(this.props.selectedPlanList, (param) => {
                ...
                actions.multitop.updateState({ planformObj: _planformObj, showLoanModal: true });
            });
        }
    }
    //子表删除数据
    del = () => {
        if (this.state.activeKey == '1') {
            multiRecordOper(this.props.selectedPlanList, (param) => {
                ...
                actions.multitop.updateState({ list2: newlist, selectedPlanList: [] });
            });
        }
    }
    ...
    getSelectedDataFunc = (selectedPlanList, record, index) => {
        if (this.state.activeKey == '1') {
            ...
            actions.multitop.updateState({ list2: _list2, selectedPlanList: _selectedPlanList });
        }
    }
